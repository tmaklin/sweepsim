...
void SampleReads(std::unique_ptr<std::istream> &strand, const long unsigned &proportion, std::unique_ptr<std::ostream> &outfile, std::vector<long unsigned> read_ids) {
...
void MixReads(std::unique_ptr<std::istream> infiles[][2], const std::vector<double> &props, const std::vector<long unsigned> read_counts, const long unsigned &n_reads, std::unique_ptr<std::ostream> outfiles[2]) {
  ...
  for (size_t i = 0; i < props.size(); ++i) {
    if (proportions[i] > 0) {
      ...
      SampleReads(infiles[i][0], prop, outfiles[0], read_id);
      SampleReads(infiles[i][1], prop, outfiles[1], read_id);
    }
  }
}
