...
void PrintHelpMessage() {
  std::cerr << "Usage: mix_reads -o <outputFile> -f <paired-endFastqFiles> [OPTIONS]\n"
	    << "Mixes sequencing reads from multiple files together; randomly or according to some proportions..\n\n"
	    << "Options:\n"
    	    << "\t-o <outputFile>\n"
	    << "\tOutput mixed reads to this file.\n"
	    << "\t-f <paired-end fastq files>\n"
	    << "\tfastq-files to mix from.\n"
    	    << "\t--props <nrIterations>\n"
	    << "\tUse preset proportions rather (default: randomly drawn from uniform distribution and normalized).\n"
